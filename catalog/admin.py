...
class SectionAdmin(admin.ModelAdmin):
  ...
  list_display = ('course_section', 'section_id', 'course', 'prof', 'start_date', 'end_date')
  # inlines = (EnrollmentInline,)
  ...
  def section_gradebook_csv(self, request, queryset):
    ...
    filename = "{!s}-{!s}gradebook-{}".format(slugify(course), title, date.strftime("%d_%m_%y"))
    ...
    for obj in queryset:
      ...
      section_problems = list(chain.from_iterable(ps.problems.all() for ps in section_problemsets))
      ...
      for user in obj.enrolled.all():
        ...
        result_dict = {(problem.title, problem.latest_score_with_usr(obj, user)) for problem in section_problems}
